...
namespace IngameScript
{
    partial class Program
    {
        ...
        /**
         * Пример:
         * Main(): ctx.putForce("arg", argument);
         * 
            ...
            public override string exec()
            {
                if (!(activeItem is Menu))
                {
                    ...
                    if (focusedItem == null && items.Count > 0) // Если не заполнено focusedItem - ищем выбранный
                    {
                        ...
                        if (a == null)
                        { // если выбранного нет - выбираем первый попавшийся
                            focusedItem = items.First();
                            changeFocusStatus(ref focusedItem);
                        }
                    }
                    if (ctx.get("arg").ToString() == "exec")
                    { // если команда на выполнение - активируем/деактивируем
                ...
                while (q.Count > 1)
                { // 1 - потому что у него последняя - пустая строка
        ...
        public class ChangeVarMenuItem : SimpleMenuItem
        {
            ...
            /// <param name="action">Действие, выполняемое при активации.</param>
