namespace SpaceEngineers.TerraNova.resourcesScript {
    public class Program {
        
        
        
        
        /* Automatic LCDs 2 Rus. v: 2.0176 [1.194] - внутриигровой сценарий от MMaster, перевёл на русский AlERK.
*
* Последнее обновление: исправлены угловые ЖК-дисплеи, отображающие только одну строку
*
* Добавлена поддержка новых предметов и блоков
* Исправлен многострочный маркер команды
* Более медленное обновление команды в SlowMode
* Последнее обновление: добавлены варианты NB (без столбцов), NN (без чисел) и NNB (без столбцов или чисел) Inventory, InvList, Missing и все их варианты (см. Руководство)
* Последнее обновление: исправлено падение скрипта при удалении блока с LCD (например, кабины)
...
* Исправлена ошибка, из-за которой пространство с правой стороны ЖК-дисплея было слишком большим по сравнению с левой стороной.
* Скрипт теперь корректно использует свойство Text Padding на ЖК-дисплее (вы можете установить его так, как хотите, и скрипт будет работать с ним)
* Оптимизация для серверов, на которых запущен скрипт ограничителя - используйте SlowMode!
* Добавлена настройка SlowMode для значительного замедления работы скрипта (обработка в 4-5 раз меньше в секунду)
* Теперь с помощью MDK!
* Первая часть переписывания управления памятью, которая должна оптимизировать распределение памяти (больше оптимизаций в процессе). */

/* Настройка: */

// Используйте этот тег для идентификации ЖК-дисплеев, управляемых этим скриптом
// Здесь можно использовать правила фильтрации имен, чтобы вы могли использовать даже G: Group или T: [My LCD]
public string LCD_TAG = "T:[LCD]";

// Установите в true, если вы хотите замедлить работу скрипта
public const bool SlowMode = false;

// Сколько строк прокручивать за шаг
...
ВИДЕО ГАЙД (От автора)
Пожалуйста, посмотрите видео, даже если вы не понимаете английский, вы можете увидеть, как там все делается.

https://youtu.be/vqpPQ_20Xso

Пожалуйста, внимательно прочитайте ПОЛНОЕ РУКОВОДСТВО, прежде чем задавать вопросы, мне пришлось удалить руководство отсюда, чтобы добавить больше возможностей :(
Пожалуйста, НЕ публикуйте этот сценарий или его производные без моего разрешения! Не стесняйтесь использовать его в чертежах!

Watch Twitter: https://twitter.com/MattsPlayCorner
and Facebook: https://www.facebook.com/MattsPlayCorner1080p

Если вы хотите создавать сценарии для космических инженеров, посмотрите MDK от Malware:
...
void ItemsConf() {
    ...
    // VANILLA ITEMS
    Add("Stone", "Ore", 100000, "Камень");
    Add("Iron", "Ore", 100000, "Железо");
    Add("Nickel", "Ore", 100000, "Никель");
    Add("Cobalt", "Ore", 100000, "Кобальт");
    Add("Magnesium", "Ore", 100000, "Магний");
    Add("Silicon", "Ore", 100000, "Кремний");
    Add("Silver", "Ore", 100000, "Серебро");
    Add("Gold", "Ore", 100000, "Золото");
    Add("Platinum", "Ore", 100000, "Платина");
    Add("Uranium", "Ore", 100000, "Уран");
    Add("Ice", "Ore", 100000, "Лёд");
    Add("Scrap", "Ore", 0, "Металлолом");	
    Add("Stone", "Ingot", 40000, "Гравий");
    Add("Iron", "Ingot", 300000, "Железо");
    Add("Nickel", "Ingot", 900000, "Никель");
    Add("Cobalt", "Ingot", 120000, "Кобальт");
    Add("Magnesium", "Ingot", 80000, "Магний");
    Add("Silicon", "Ingot", 80000, "Кремний");
    Add("Silver", "Ingot", 800000, "Серебро");
    Add("Gold", "Ingot", 80000, "Золото");
    Add("Platinum", "Ingot", 45000, "Платина");
    Add("Uranium", "Ingot", 12000, "Уран");
    Add("AutomaticRifleItem", "Tool", 0, "Автомат");
    Add("PreciseAutomaticRifleItem", "Tool", 0, "* Точный автомат");
    Add("RapidFireAutomaticRifleItem", "Tool", 0, "** Скорострельный автомат");
    Add("UltimateAutomaticRifleItem", "Tool", 0, "*** Элитный автомат");
    Add("WelderItem", "Tool", 0, "Автоген");
    Add("Welder2Item", "Tool", 0, "* Улучшенный автоген");
    Add("Welder3Item", "Tool", 0, "** Профессиональный автоген");
    Add("Welder4Item", "Tool", 0, "*** Элитный автоген");
    Add("AngleGrinderItem", "Tool", 0, "Болгарка");
    Add("AngleGrinder2Item", "Tool", 0, "* Улучшенная болгарка");
    Add("AngleGrinder3Item", "Tool", 0, "** Профессиональная болгарка");
    Add("AngleGrinder4Item", "Tool", 0, "*** Элитная болгарка");
    Add("HandDrillItem", "Tool", 0, "Отбойник");
    Add("HandDrill2Item", "Tool", 0, "* Улучшенный отбойник");
    Add("HandDrill3Item", "Tool", 0, "** Профессиональный отбойник");
    Add("HandDrill4Item", "Tool", 0, "*** Элитный отбойник");
    Add("Construction", "Component", 50000, "Стройкомпоненты");
    Add("MetalGrid", "Component", 15500,  "Компонет решётки");
    Add("InteriorPlate", "Component", 55000, "Внутренняя пластина");
    Add("SteelPlate", "Component", 300000, "Стальная пластина");
    Add("Girder", "Component", 3500, "Балка");
    Add("SmallTube", "Component", 26000, "Малая труба");
    Add("LargeTube", "Component", 6000, "Большая труба");
    Add("Motor", "Component", 16000, "Мотор");
    Add("Display", "Component", 500, "Экран");
    Add("BulletproofGlass", "Component", 12000, "Бронированное стекло");
    Add("Computer", "Component", 6500, "Компьютер");
    Add("Reactor", "Component", 10000, "Компоненты реактора");
    Add("Thrust", "Component", 16000, "Детали ускорителя");
    Add("GravityGenerator", "Component", 250, "Гравикомпоненты");
    Add("Medical", "Component", 120, "Медкомпоненты");
    Add("RadioCommunication", "Component", 250, "Радиокомпоненты", "radio");
    Add("Detector", "Component", 400, "Компоненты детектора");
    Add("Explosives", "Component", 500, "Взрывчатка");
    Add("SolarCell", "Component", 2800, "Солнечная ячейка");
    Add("PowerCell", "Component", 2800, "Энергоячейка"); 
    Add("Superconductor", "Component", 3000, "Сверхпроводник");
    Add("Canvas", "Component", 300, "Полотно парашюта");
	/* Add("ZoneChip", "Component", 100, "Зональный чип"); */
    Add("Datapad", "Datapad", 0, "Инфопланшет");
    Add("Medkit", "ConsumableItem", 0, "Аптечка");
    Add("Powerkit", "ConsumableItem", 0, "Внешний аккумулятор");
    Add("SpaceCredit", "PhysicalObject", 0, "Кредиты");
    Add("NATO_5p56x45mm", "Ammo", 8000, "5.56x45мм", "5.56x45mm");
    Add("NATO_25x184mm", "Ammo", 2500, "25x184мм", "25x184mm");
    Add("Missile200mm", "Ammo", 1600, "200мм ракета", "200mmmissile");
    Add("OxygenBottle", "OxygenContainerObject", 5, "Кислородные баллоны"); 
    Add("HydrogenBottle", "GasContainerObject", 5, "Водородные баллоны"); 
    ...
    // AzimuthSupercharger
    Add("AzimuthSupercharger", "Component", 1600, "Нагнетатель", "supercharger", false);
    // OKI Ammo
...
(ϔ.Ć(Ą)){ƚ.Ȣ();return;}}ϔ.Ğ=0;ƚ.ȡ();}catch(Exception ex){Echo("ОПИСАНИЕ ОШИБКИ:\n"+ex.ToString());Me.Enabled=false;}}
...
.ɣ.Count*100;n.Ʈ(ʒ);if(ϗ==n.ɣ.Count){n.Ǎ("");n.Ƣ("Automatic LCDs 2 Rus");n.Ƣ("от MMaster / AlERK");}else n.ǌ(n.ɣ[ϗ]);bool ß=k.ß;k.ß=
...
// amount
{ "A1", "Пусто" }, 
{ "ALT1", "Абсолютная высота: "}, 
{ "ALT2", "Высота полета: "}, 
{ "B1", "ЗАГРУЗКА..."}, 
{ "C1", "Всего: " }, 
{ "C2", "Объем груза:" }, 
{ "C3", "Формат отсчета неверный, используйте: " },
{ "C4", "ПРОСРОЧЕН" },
{ "C5", "дней" }, 
// customdata
{ "CD1", "Блок не найден: " }, // NEW 
{ "CD2", "Не введено ИМЯ" }, // NEW 
{ "D1", "Необходимо ввести ИМЯ." },
{ "D2", "Блок не найден." }, 
{ "D3", "Повреждённых блоков нет." }, 
{ "DO1", "Соединители не найдены." }, // NEW
{ "DTU", "Неправильный GPS Формат" }, 
{ "GA", "Иск.Гр"}, // (not more than 5 characters) 
{ "GN", "Природная.Гр"}, // (not more than 5 characters) 
{ "GT", "Общая.Гр"}, // (not more than 5 characters) 
{ "G1", "Общая гравитация: "}, 
{ "G2", "Природная гравитация: "}, 
{ "G3", "Искусственная гравитация: "}, 
{ "GNC", "Нет кабины!"},
{ "H1", "Напишите команды для пользовательских данных этой панели." },
// inventory
{ "I1", "Руды" }, 
{ "I2", "в наличии" }, 
{ "I3", "Руда" }, 
...
{ "I6", "Баллоны" }, 
{ "I7", "Патроны" }, 
{ "I8", "Инструменты и Оружие" }, 
{ "M1", "Масса груза: " },
// oxygen
{ "O1", "Разгерметизация" }, 
{ "O2", "Кислородная Ферма" }, 
{ "O3", "Производство кислорода НЕТ." }, 
{ "O4", "Кислородный бак" }, 
// position
{ "P1", "Блок не найден" }, 
{ "P2", "Координаты" }, 
...
{ "P4", "Потребление" }, 
{ "P5", "Зарядка" }, 
{ "P6", "Генераторов энергии НЕТ!" }, 
{ "P7", "Батареи" }, 
{ "P8", "Общее потребление" }, 
{ "P9", "Реакторы" }, 
{ "P10", "СОЛ.Панели" }, 
{ "P11", "Мощность" },
{ "P12", "Двигатели" }, // NEW!
...
{ "PT1", "Время работы: " }, 
{ "PT2", "Время зарядки: " }, 
{ "PU1", "Энергопотребление: " }, 
{ "S1", "Скорость:" }, 
{ "SM1", "Масса корабля:" }, 
{ "SM2", "Базовая масса:" }, 
{ "SD", "Дистанция остановки: " }, 
{ "ST", "Время остановки: " }, 
// text
{ "T1", "Источник не найден: " }, 
{ "T2", "Неправильное Имя" },
// tanks
{ "T4", "Ошибка в названии. прим: 'Tanks * Hydrogen'" }, 
{ "T5", "нет {0} такого бака." }, // {0} is tank type 
{ "UC", "Неизвестная Команда" }, 
// occupied & dampeners
{ "SC1", "Не найден блок управления." }, 
{ "SCD", "Демпфер: " },
{ "SCO", "Занятый: " },
// working
{ "W1", "Выключено" }, 
